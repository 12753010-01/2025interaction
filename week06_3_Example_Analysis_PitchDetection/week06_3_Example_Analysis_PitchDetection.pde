...
//想要把聲音最重要的頻率(Hz)找出來，使用PitchDetector物件
...
SoundFile sound;//新加這一行，把beat.mp3讀入
...
void setup() {
  ...
  // capture audio input
  //pitchDetector.input(new AudioIn(this, 0));// 原本程式，是用Audio In音效卡的音源輸入
...
void draw() {
  ...
  if (pitchAndConfidence[1] > 0) {
    ...
    println(pitchAndConfidence[0],pitchAndConfidence[1]);//把pitch和confidence都印出來
  }
